...
public class UserManagementController {
  ...
  @GetMapping
  @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ACM_HIGH_BOARD')")
  public ResponseEntity<Page<UserDTO>> searchUsers(
      @RequestParam(required = false) String query,
      @RequestParam(required = false) Role role,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "10") int size) {
    Page<UserDTO> users = userService.searchUsers(query, role, page, size);
  ...
  public ResponseEntity<?> assignAssociations(@PathVariable UUID id, @RequestBody AssignAssociationsRequest request) {
    try {
      UserDTO updated = userService.assignCommitteeAndClubs(id, request.getCommitteeId(), request.getClubIds());
      return ResponseEntity.ok(updated);
    } catch (Exception e) {
      return ResponseEntity.badRequest().body(new ErrorMessageResponse(e.getMessage()));
    }
