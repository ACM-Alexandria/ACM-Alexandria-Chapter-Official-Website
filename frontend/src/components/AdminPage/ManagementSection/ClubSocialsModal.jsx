...
const ClubSocialsModal = ({ open, onClose, club, onSaved }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-[fadeIn_0.2s_ease]">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl dark:shadow-slate-950/60 max-w-md w-full max-h-[85vh] overflow-y-auto p-6 animate-[scaleIn_0.25s_ease] flex flex-col">
        {/* Modal Header */}
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                    ...
                    {links.map((link, index) => (
                      <div key={index} className="flex gap-2 items-center">
                        <input
                          ...
                          className="flex-1 px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 text-xs font-semibold rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4B98C8]/25 focus:border-[#4B98C8] transition-all"
                        />
                        <button
                          type="button"
                ...
                <button
                  ...
                  className="px-4 py-2.5 border border-dashed border-slate-300 dark:border-slate-600 text-slate-500 dark:text-slate-300 hover:text-slate-700 dark:hover:text-slate-100 text-xs font-bold rounded-xl w-full flex items-center justify-center gap-1.5 transition-all hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95"
                >
