 ...
 * Hover-synced legend · center HUD · ring arc segments
 */
const DepartmentChart = ({ data = {} }) => {
  ...
  const arcs = list.map((d, i) => {
    ...
    const rot = offset;
    offset += frac * 360;
    return { ...d, frac, dash, gap, rot, color: CHART_COLORS[i % CHART_COLORS.length] };
  });
      ...
      <div className="flex-1 flex flex-col items-center gap-4">
        ...
        <div className="w-full flex flex-col gap-1 max-h-36 overflow-y-auto">
          {arcs.map((a, i) => (
            <div
              key={i}
              className={`flex items-center justify-between px-3 py-1.5 rounded-lg cursor-pointer transition-all duration-200 ${hovered?.name === a.name ? "bg-slate-50" : "hover:bg-slate-50/60"
                }`}
              onMouseEnter={() => setHovered(a)}
              onMouseLeave={() => setHovered(null)}
            >
