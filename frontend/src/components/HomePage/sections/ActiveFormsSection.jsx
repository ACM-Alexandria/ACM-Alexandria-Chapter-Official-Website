        ...
        return (
            <div className="mt-24 w-full relative overflow-hidden" id="exclusive-forms">
                <div className="max-w-7xl mx-auto space-y-6 animate-pulse">
                    <div className="h-12 bg-slate-100 dark:bg-slate-800 rounded-full w-80 mx-auto" />
                    <div className="h-4 bg-slate-100 dark:bg-slate-800 rounded-full w-64 mx-auto mt-4" />
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-16">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-48 bg-slate-50 dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700" />
                        ))}
                    </div>
                </div>
            </div>
        ...
        <div className="mt-24 w-full relative overflow-hidden" id="exclusive-forms">
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="text-center mb-20">
                    <h2 
                        className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight mb-6 flex items-center justify-center gap-3"
                        data-aos="fade-up"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {activeForms.map((form, idx) => (
                        <div 
                            ...
                            className="group relative bg-white dark:bg-slate-800 rounded-[2rem] p-8 border border-slate-100/60 dark:border-slate-700 shadow-lg shadow-slate-200/40 dark:shadow-slate-950/40 hover:shadow-2xl hover:shadow-[#4B98C8]/20 transition-all duration-500 hover:-translate-y-2 flex flex-col cursor-pointer overflow-hidden"
                            data-aos="fade-up"
                        ...
                        >
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                ...
                                <h3 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 mb-4 line-clamp-2 leading-tight group-hover:text-[#205E85] transition-colors duration-300">
                                    {form.title}
                                </h3>
                                {form.description && (
                                    <p className="text-slate-500 dark:text-slate-300 text-base font-medium line-clamp-3 leading-relaxed mb-8 flex-grow">
                                        {form.description}
                                    </p>
                                )}
                                
                                <div className="mt-auto flex items-center justify-between text-[#4B98C8] font-bold text-sm uppercase tracking-widest pt-4 border-t border-slate-100 dark:border-slate-700 group-hover:border-[#4B98C8]/20 transition-colors duration-300">
                                    <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">View Details</span>
                                    <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-700 group-hover:bg-[#4B98C8]/10 flex items-center justify-center transition-all duration-300 transform group-hover:translate-x-2">
